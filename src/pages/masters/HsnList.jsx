...
export default function HsnList() {
  ...
  const fetchHsns = useCallback(async () => {
    try {
      ...
      if (debouncedSearch.trim() !== "") {
        ...
        result = await getHsns({ search: debouncedSearch });
      } else {
        // Normal paginated mode
        result = await getHsns({ limit, page, search: "" });
      }
      ...
      if (result?.success) {
        ...
        setError(result?.message || "Failed to load HSN codes");
        setHsns([]);
        setTotalRecords(0);
        setTotalPages(1);
      }
    } catch (error) {
      console.error("Fetch HSN codes error:", error);
      setHsns([]);
      setTotalRecords(0);
      setTotalPages(1);
  ...
  const handleDelete = async (id, code) => {
    if (!window.confirm(`Delete HSN code "${code}"? This action cannot be undone.`)) return;
    const result = await deleteHsn(id);
    if (result.success) {
      if (hsns.length === 1 && page > 1 && !debouncedSearch) {
        setPage(page - 1);
      } else {
        fetchHsns();
      }
    } else {
      setError(result?.message || "Failed to delete HSN code");
    }
  };
  ...
  return (
    <div className="p-6">
        ...
        <div>
          ...
          {!loading && totalRecords > 0 && !isSearchActive && (
            <p className="text-sm text-gray-400 mt-0.5">
              {totalRecords} {totalRecords === 1 ? "code" : "codes"} total
            </p>
          )}
          {isSearchActive && hsns.length > 0 && (
            <p className="text-sm text-gray-400 mt-0.5">
              Found {hsns.length} {hsns.length === 1 ? "code" : "codes"}
            </p>
          )}
        </div>
            ...
            {loading ? (
              ...
              <tr>
                <td colSpan="3" className="px-6 py-12 text-center text-gray-500">
                  {isSearchActive 
                    ? `No HSN codes found matching "${search}". Try a different search term.`
      ...
      {isSearchActive && hsns.length === 0 && !loading && (
