...
function BumDropdown({ bums, selectedIds, onChange }) {
  ...
  // Prevent any mousedown inside the dropdown from bubbling to the outside-click handler
  const stopPropagation = (e) => e.stopPropagation();

  const selectedBums = bums.filter((b) => selectedIds.includes(b.id));
  const totalDelivery = selectedBums.reduce(
    (acc, b) => acc + b.items.filter((i) => i.deliver).length, 0
  );
  ...
  return (
    <div ref={ref} className="relative w-full">
      ...
      <button
        ...
        className="w-full flex items-center justify-between border border-gray-300 rounded-xl px-4 py-3 bg-white hover:border-violet-400 transition-colors shadow-sm"
      >
        {selectedIds.length === 0 ? (
          <span className="text-sm text-gray-400">Select component group(s) (BUM)…</span>
        ) : (
          <div className="flex items-center gap-2 min-w-0 flex-wrap">
            {selectedBums.slice(0, 4).map((b) => (
              <span
                key={b.id}
                className="inline-flex items-center gap-1 bg-violet-100 text-violet-700 text-xs font-semibold px-2 py-0.5 rounded-md"
              >
                {b.code}
                <button
            ...
            {selectedIds.length > 4 && (
              <span className="text-xs text-gray-400">+{selectedIds.length - 4} more</span>
            )}
            {totalDelivery > 0 && (
              <span className="text-xs text-green-600 font-semibold ml-1">
                · {totalDelivery} for delivery
              </span>
            )}
          </div>
        )}
        <div className="flex items-center gap-1.5 flex-shrink-0 ml-3">
          {selectedIds.length > 0 && (
            <span className="text-xs bg-violet-600 text-white font-bold px-1.5 py-0.5 rounded-full">
              {selectedIds.length}
            </span>
          )}
          <ChevronDown
      ...
      {open && (
        <div
          className="absolute z-30 w-full mt-1.5 bg-white border border-gray-200 rounded-xl shadow-2xl overflow-hidden"
        ...
        >
          ...
          <div className="flex items-center gap-2 px-3 py-2.5 border-b border-gray-100 bg-gray-50">
            ...
            <div className="flex items-center gap-2 border-l border-gray-200 pl-2 ml-1">
              <button
                onClick={() => onChange(bums.map((b) => b.id))}
          ...
          <div className="max-h-72 overflow-y-auto">
            ...
            {filtered.map((bum) => {
              const isSelected = selectedIds.includes(bum.id);
              const deliverCnt = bum.items.filter((i) => i.deliver).length;
              return (
                <button
                  key={bum.id}
                  onClick={() => toggle(bum.id)}
                  className={`w-full flex items-center gap-3 px-4 py-3 border-b border-gray-100 last:border-0 text-left transition-colors hover:bg-gray-50 ${
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-400 mt-0.5">
                      {bum.sub} · {bum.items.length} items
                    </p>
                  </div>
          ...
          {/* Footer */}
          {selectedIds.length > 0 && (
            <div className="px-4 py-2.5 bg-violet-50 border-t border-violet-100 flex items-center justify-between">
    ...
    >
      ...
      <td className="px-3 py-2.5 w-36">
        <div className="flex items-center justify-center gap-1">
          <button
            onClick={() => handleQty(Math.max(0, item.qty - 1))}
      ...
      <td className="px-3 py-2.5 w-14 text-center text-xs text-gray-500 font-medium">
        {item.unit}
      </td>
...
// ── BUM Panel (one per selected BUM) ─────────────────────────────────────────
function BumPanel({ bum, orderId, dispatch, navigate }) {
  ...
  const visibleItems = bum.items.filter((i) => matchFilter(i) && matchSearch(i));
  const allDelivering =
    bum.items.length > 0 && bum.items.every((i) => i.deliver);

  const toggleBumAllDeliver = () =>
    dispatch({ type: "TOGGLE_BUM_ALL_DELIVER", orderId, bumId: bum.id });
  ...
  return (
    <div className="border border-gray-200 rounded-xl overflow-hidden mb-3 last:mb-0">
      ...
      >
        ...
        <StatusBadge status={bum.status} />
        <button
          onClick={() => navigate(`/orders/${orderId}/bum/${bum.id}`)}
        ...
        <table className="w-full min-w-[680px]">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-200">
              <th className="w-10 px-4 py-2.5 text-center text-xs font-semibold text-gray-400 uppercase tracking-wide">
                ✓
              </th>
              <th className="text-left px-3 py-2.5 text-xs font-semibold text-gray-500 uppercase tracking-wide">
                Item
              </th>
              <th className="text-center px-3 py-2.5 text-xs font-semibold text-gray-500 uppercase tracking-wide w-36">
                Quantity
              </th>
              <th className="text-center px-3 py-2.5 text-xs font-semibold text-gray-500 uppercase tracking-wide w-14">
                Unit
              </th>
              <th className="text-right px-3 py-2.5 text-xs font-semibold text-gray-500 uppercase tracking-wide w-36">
                Status
              </th>
              <th className="text-center px-3 py-2.5 text-xs font-semibold text-gray-500 uppercase tracking-wide w-28">
                Deliver
              </th>
      ...
      <div className="px-5 py-3 bg-gray-50 border-t border-gray-100 flex items-center justify-between">
        <p className="text-xs text-gray-500">
          {bum.items.filter((i) => i.deliver).length > 0
            ? `${bum.items.filter((i) => i.deliver).length} items marked for delivery`
...
export default function OrderDetailPage() {
  ...
  const [selectedBumIds, setSelectedBumIds] = useState(
    order?.bums[0]?.id ? [order.bums[0].id] : []
  );
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-4">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          <div className="flex items-start gap-4">
            ...
            <div className="flex flex-col items-end gap-2 flex-shrink-0">
              ...
              <button className="px-3 py-1 border border-gray-200 rounded-full text-xs text-gray-600 hover:bg-gray-50">
                Not prioritized ★
              </button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-5 pt-4 border-t border-gray-100">
            ...
            <div>
              ...
              <input
                type="date"
                className="text-sm text-gray-600 bg-transparent border-0 p-0 focus:outline-none cursor-pointer"
              />
            </div>
            <div>
        ...
        {/* ── Remarks — label left, chips inline right ── */}
        <div className="bg-white rounded-xl border border-gray-200 px-4 py-3">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {order.remarks.map((r, i) => (
              <div
                key={i}
                className="flex items-center gap-1.5 text-xs text-gray-700 bg-yellow-50 border border-yellow-200 rounded-lg px-2.5 py-1.5"
              >
                <span>{r}</span>
                <button
                  onClick={() =>
                    dispatch({ type: "REMOVE_REMARK", orderId: order.id, index: i })
                  }
            ...
            {addingRemark ? (
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={() => { setAddingRemark(false); setRemarkText(""); }}
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          ...
          {tableOpen && (
            <>
              ...
              {/* Render a panel per selected BUM */}
              <div className="p-4">
                {selectedBums.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-16 text-gray-400">
                    <Package size={36} className="mb-3 opacity-30" />
                    <p className="text-sm">
                      Select one or more BUM groups from the dropdown above
                    </p>
                  </div>
                ) : (
            ...
            <div className="px-5 py-3 flex items-center gap-3 flex-wrap bg-gray-50">
              {order.bums.map((bum) => (
                <button
                  key={bum.id}
                  onClick={() => {
                    setSelectedBumIds([bum.id]);
                    setTableOpen(true);
                  }}
                  className="flex items-center gap-1.5 text-xs text-gray-600 hover:text-violet-600 transition-colors"
                >
                  <span className="w-5 h-5 rounded bg-gray-200 flex items-center justify-center text-xs font-bold">
                    {bum.code}
                  </span>
                  <span className="truncate max-w-[120px]">
                    {bum.name.split(":")[1]?.trim() || bum.name}
                  </span>
                  {bum.items.filter((i) => i.deliver).length > 0 && (
